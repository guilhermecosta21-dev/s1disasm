...
namespace S1ObjectDefinitions.GHZ
{
	class SwingingPlatform : ObjectDefinition
	{
		...
		{
			imgwreckingball = ObjectHelper.MapASMToBmp(ObjectHelper.OpenArtFile("../../artkospm/GHZ Giant Ball.kospm", CompressionType.KosinskiPlusM), "../../_maps/GHZ Ball.asm", 1, 2);
			for (int i = 0; i < labels.Length; i++)
				imgs.Add(ObjectHelper.MapASMToBmp(ObjectHelper.OpenArtFile("../../artkospm/GHZ Swinging Platform.kospm", CompressionType.KosinskiPlusM), "../../_maps/Swinging Platforms (GHZ).asm", labels[i], i == 1 ? 0 : 2));
		}
